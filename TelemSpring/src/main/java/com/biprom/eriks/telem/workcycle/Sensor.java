...
public class Sensor implements Runnable {
	...
	private WorkcycleRunner workcycleRunner;
	...
	public Sensor(SensorService sensorService, WorkcycleRunner workcycleRunner) {

		this.sensorService = sensorService;
		this.workcycleRunner = workcycleRunner;
	}
	...
	public void run() {
		...
		while (run) {
			try {
				...
				if (workcycleRunner.isRunning()) {

					Date now = new Date();

					final double s0 = analog_input_card_1_1.read_raw(0X6C, 0, 1, 0, 0) * 0.05456766; // flow
																										// 18

					final double s1 = 0;// analog_input_card_1_1.read_raw(0X6C,
										// 1,
										// 1, 0, 0);// flow 29

					final double s2 = ((0.19 * (analog_input_card_1_1.read_raw(0X6C, 2, 0, 1, 0) - 736))); // eds13

					final double s3 = (((analog_input_card_2_1.read_raw(0X6D, 0, 1, 0, 0)) - 365.04) / 19.16); // temp
																												// olie
																												// na
																												// pomp

					final double s4 = (((analog_input_card_1_1.read_raw(0X6C, 3, 1, 0, 0)) - 365.04) / 19.16); // temp
																												// olie
																												// input
					// 03
					...
					// 20
					final double s8 = (((((11.179 / 2047) * analog_input_card_3_1.read_raw(0X6E, 0, 0, 0, 0))) * 3)
							- 3); // stauf
					// nas1
					final double s9 = (((((11.179 / 2047) * analog_input_card_3_1.read_raw(0X6E, 1, 0, 0, 0))) * 3)
							- 3); // stauf
					// nas
					// 2
					final double s10 = (((((11.179 / 2047) * analog_input_card_3_1.read_raw(0X6E, 2, 0, 0, 0))) * 3)
							- 3); // stauf
					// nas
					// 3
					final double s11 = ((((11.179 / 2047) * analog_input_card_3_1.read_raw(0X6E, 3, 0, 0, 0)) - 1.0)
							/ 0.08); // RHOlie
